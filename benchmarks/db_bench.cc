...
static bool FLAGS_activate_rallydb = true;
...
class Benchmark {
 ...
 private:
  ...
  void Open() {
    ...
    options.activate_rallydb = FLAGS_activate_rallydb;
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    }else if (sscanf(argv[i], "--use_rallydb=%d%c", &n, &junk) == 1 &&
               (n == 0 || n == 1)) {
      FLAGS_activate_rallydb = n;
