...
def run_train_lstm():
    ...
    torch.save(net.state_dict(), '{}/net.pt'.format(mod_dir))
    print("Save in:", '{}/net.pt'.format(mod_dir))

    model_data = batch_var_x
    # Use torch.jit.trace to generate a torch.jit.ScriptModule via tracing.
    traced_script_module = torch.jit.trace(net, model_data)

    traced_script_module.save('{}/model.pt'.format(mod_dir))

    '''eval'''
    net.load_state_dict(torch.load('{}/net.pt'.format(mod_dir), map_location=lambda storage, loc: storage))
    net = net.eval()
