...
class Version {
 public:
  ...
  int GetAccessTimeFromNewSSTable(int level);
  ...
  void AdjustBloomFilter(TableCache *cache, SequenceNumber current_time, uint64_t old_time, 
   uint64_t access_time,  const std::atomic<uint64_t>& number);
 ...
 private:
  ...
  int64_t GetAdjustMemory(uint64_t output);

  int64_t GetAdjustMemory(uint64_t old_access, uint64_t new_access, const std::atomic<uint64_t>& number);
