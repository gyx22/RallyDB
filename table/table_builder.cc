...
Status TableBuilder::Finish() {
  ...
  if (ok()) {
    ...
    std::string kv_number_key = "leveldb.kv.number";
    if (r->filter_block != nullptr) {
      // Add mapping from "filter.Name" to location of filter data
      std::string filter_key = "filter.";
      filter_key.append(r->options.filter_policy->Name());

      // insert filter and kv number kv pair by ordered according to comparator  
      const Comparator *comparator = rep_->options.comparator;
      if(comparator->Compare(filter_key, kv_number_key) < 0){
        meta_index_block.Add(filter_key, filter_block_meta);
        meta_index_block.Add(kv_number_key, std::to_string(rep_->num_entries));
      }else{
        meta_index_block.Add(kv_number_key, std::to_string(rep_->num_entries));
        meta_index_block.Add(filter_key, filter_block_meta);
      }
    }else{ // just insert kv number
      meta_index_block.Add(kv_number_key, std::to_string(rep_->num_entries));
    }
